...
fn main() -> Result<()> {
    ...
    io::stdin().read_to_string(&mut buffer)?;use pest::iterators::{Pair, Pairs};
use pest::Parser;
use pest_derive::Parser;
use std::collections::HashMap;
use std::io::{self, Read};
...
enum Value {
    ...
    Function {
        params: Vec<String>,
        body: Vec<Pair<'static, Rule>>,  // Stored as AST pairs for execution
    },
    // Add more: List, etc.
}
...
struct Interpreter {
    ...
    functions: HashMap<String, Value>,  // Separate for functions
}
...
impl Interpreter {
    ...
    fn execute(&mut self, code: &str) -> Result<()> {
        let parse_result = NotScriptingParser::parse(Rule::program, code)
            .context("Failed to parse program")?;
        let statements = parse_result.flatten().filter(|p| p.as_rule() == Rule::statement);
    ...
    fn exec_statement(&mut self, pair: Pair<Rule>) -> Result<()> {
        match pair.as_rule() {
    ...
    fn exec_var_decl(&mut self, pair: Pair<Rule>) -> Result<()> {
    ...
    fn exec_assign(&mut self, pair: Pair<Rule>) -> Result<()> {
        let mut inner = pair.into_inner();
        let ident = inner.next().unwrap().as_str().to_string();
        let _eq = inner.next();
        let expr_pair = inner.next().unwrap();
    ...
    fn exec_print(&mut self, pair: Pair<Rule>) -> Result<()> {
        let mut inner = pair.into_inner();
    ...
    fn exec_if(&mut self, pair: Pair<Rule>) -> Result<()> {
        let mut inner = pair.into_inner();
        let _if = inner.next();
    ...
    fn exec_loop(&mut self, pair: Pair<Rule>) -> Result<()> {
        let mut inner = pair.into_inner();
        let keyword = inner.next().unwrap().as_str();
        match keyword {
            "loop" => {
            ...
            "for" => {
                ...
                let block = inner.next().unwrap();
                // Assume range_expr is like 0..10 or list
                // For simplicity, assume int range like "0..10"
    ...
    fn exec_fn_decl(&mut self, pair: Pair<Rule>) -> Result<()> {
        let mut inner = pair.into_inner();
        ...
        let block = inner.next().unwrap();
        let body: Vec<Pair<Rule>> = block.into_inner().collect();
        self.functions.insert(name, Value::Function { params, body });
        Ok(())
    }

    fn exec_fn_call(&mut self, pair: Pair<Rule>) -> Result<()> {
        let mut inner = pair.into_inner();
        let name = inner.next().unwrap().as_str().to_string();
        ...
        // Discard result unless assigned
        let _ = result;
        Ok(())
    }
    ...
    fn call_function(&mut self, name: &str, args: Vec<Value>) -> Result<Value> {
        if let Some(Value::Function { params, body }) = self.functions.get(name).cloned() {
            ...
            Ok(Value::Bool(true))  // Placeholder return, add return stmt later
        } else {
            bail!("Unknown function: {}", name);
        }
    }

    fn exec_command(&mut self, pair: Pair<Rule>) -> Result<()> {
    ...
    fn exec_block(&mut self, pair: Pair<Rule>) -> Result<()> {
    ...
    fn eval_expr(&self, pair: Pair<Rule>) -> Result<Value> {
        match pair.as_rule() {
    ...
    fn eval_logic(&self, mut pairs: Pairs<Rule>) -> Result<Value> {
    ...
    fn eval_compare(&self, mut pairs: Pairs<Rule>) -> Result<Value> {
        let mut left = self.eval_expr(pairs.next().unwrap())?;
        while let Some(op_pair) = pairs.next() {
            let op = op_pair.as_str();
            let right = self.eval_expr(pairs.next().unwrap())?;
    ...
    fn eval_arith(&self, mut pairs: Pairs<Rule>) -> Result<Value> {
        let mut left = self.eval_expr(pairs.next().unwrap())?;
        while let Some(op_pair) = pairs.next() {
            let op = op_pair.as_str();
            let right = self.eval_expr(pairs.next().unwrap())?;
    ...
    fn eval_term(&self, mut pairs: Pairs<Rule>) -> Result<Value> {
        let mut left = self.eval_expr(pairs.next().unwrap())?;
        while let Some(op_pair) = pairs.next() {
            let op = op_pair.as_str();
            let right = self.eval_expr(pairs.next().unwrap())?;
    ...
    fn eval_literal(&self, pair: Pair<Rule>) -> Result<Value> {
        let inner = pair.into_inner().next().unwrap();
        match inner.as_rule() {
            Rule::string => Ok(Value::String(inner.as_str().trim_matches(|c| c == '"' || c == '\'').to_string())),
